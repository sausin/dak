...
- **Jev cloud classification.** When Dak's on-phone classifier cannot sort a new incoming message, Jev can send the business sender ID (a header such as "VM-HDFCBK" or a short code) and a masked copy of the message, with numbers, amounts, card numbers, links, email addresses and likely names replaced by placeholders, to Dak's classification service for a second opinion. The service returns a category and does not keep the text. It is limited to a monthly number you set. Messages from people (phone numbers) are never sent, and neither are their numbers.
