...
/**
 * Synchronous, cached phone-number → contact lookup via `PhoneLookup` (handles formatting and country-code
 * differences). Returns null without READ_CONTACTS. Call off the main thread.
 */
@Singleton
class ContactLookup @Inject constructor(@ApplicationContext private val context: Context) {
    ...
    fun isContact(address: String): Boolean = find(address) != null

    fun displayName(address: String): String? = find(address)?.displayName
