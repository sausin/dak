...
| --- | --- | --- |
| `ProviderReader` | `TelephonyProviderReader` | SMS + MMS merged newest-first by walking both cursors (no SQL LIMIT needed); name lookups, so OEM schemas without `sub_id` work (falls back to `sim_id`/slot columns, else `NO_SUB_ID`). Paging reads use minimal projections (`provider/ProviderProjections`: the mapped columns intersected with the columns an empty `_id < 0` probe reports, once per process; null projection when the probe fails); MMS parts are read per page (`mid IN (...)`), addresses per message (`content://mms/<id>/addr` has no bulk form) with a two-column projection. MMS body = text/plain parts; attachments use `content://mms/part/<id>` URIs. Filters MMS to `m_type` 128/130/132. Provider errors → empty results. Fills `Message.deliveryStatus` / `deliveredAtMillis` (see "Delivery ticks"); `outgoingStates(keys)` reads only box + delivery columns for the index's tick refresh. |
