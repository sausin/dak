 ...
 *
 * A copy of `:classify`'s `app.dak.classify.text.AnalysisText` (`:finance` does not depend on `:classify`), so the
 * transaction parser reads a body exactly as the classifier and the fake-credit detector do; `:classify`'s
 * `AnalysisTextEquivalenceTest` checks the two agree. Change both together.
 */
object AnalysisText {

    /** Combining marks kept in one run. */
    const val MAX_MARKS: Int = 8
     ...
     */
    fun capMarksKeepingOffsets(text: String): String {
        var run = 0
