...
| 5 | Provider ContentObserver → incremental reconcile | in-process only | about 600 runs (300 ms debounce, 1–2 provider changes per SMS), each walking the whole SMS table for a count | about 300 runs (3 s coalescing). The table-count walk runs at most once per 30 min | indexed `id > max` query, refresh of 20 recent rows, tick refresh of unsettled outgoing rows (last 7 days; a narrow provider query only when some await a report) | **Fixed.** Coalesced in `IndexSync`. Never keeps the process alive. |
