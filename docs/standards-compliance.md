...
| 14 | Text safety: UAX #9, UTS #39, UTS #46 | 9 | 0 | 0 | 0 | 9 |
| 15 | OWASP MASVS v2 (high level) | 8 | 0 | 0 | 0 | 8 |
| | **Total** | **110** | **24** | **2** | **19** | **155** |
...
| Concatenated SMS (UDH IEI 0x00 with 8-bit ref and IEI 0x08 with 16-bit ref, §9.2.3.24.1 / §9.2.3.24.8), reordering, duplicate segments | Framework: `InboundSmsHandler` stores segments in the `raw` table and delivers one SMS_DELIVER with every PDU in sequence order. Dak joins bodies at `IncomingSmsProcessor.kt:49` | N/A (framework `InboundSmsHandler`) | None | `sms-pdu.py --ref16 --shuffle` / `--order 3,2,1` / `--drop N` (device-test-plan 27-28). Assert one inbox row with the correct text |
...
| `scripts/sms-pdu.py`: default alphabet and extension table are correct | `android/scripts/sms-pdu.py` `GSM_BASIC` / `GSM_EXT` (index = septet value; the extension table maps `\f ^ { } \ [ ~ ] \| €`) | Compliant | None | `python3 sms-pdu.py +91… "{[€]}"`, then decode with `SmsMessage.createFromPdu` in a JVM test |
| `sms-pdu.py`: septet packing with fill bits after the UDH, UDL counted in septets including the UDH | `sms-pdu.py` `pack_septets`, `build_parts` (fill bits and part sizes follow the header length) | Compliant | None | Multipart GSM text: round-trip through `SmsMessage` |
| `sms-pdu.py`: UCS-2 handling of non-BMP characters | `sms-pdu.py` `split_ucs2` emits UTF-16 (surrogate pairs kept together, never split). Strictly, 23.038 UCS-2 has no surrogates. Android and most handsets accept UTF-16, so this matches real traffic | Partial | Document that it matches Android's behaviour. Add a `--strict-ucs2` flag that rejects non-BMP characters so strict-decoder paths can be tested | Emoji text: 70-unit boundary does not split a pair |
| `sms-pdu.py`: fixtures for the rest of 23.040/23.038 (16-bit concat refs, out-of-order or missing parts, shift-table IEIs 0x24/0x25, class 0, PID 0x40/0x41–0x47, MWI, 8-bit ports, status reports) | `--pid HEX`, `--flash`; `--ref16` (IEI 0x08) and `--ref N`; `--order 3,1,2`, `--shuffle [--seed]`, `--drop N`; `--port DEST[:SRC]` (IEI 0x05) and `--hex` 8-bit data (DCS 0x04, with or without a port); `--nls hi\|bn` with `--nls-tables both\|locking\|single` (IEI 0x24/0x25, Hindi and Bengali tables as in Android's `GsmAlphabet`); `--status-report --mr N --st HEX` (SMS-STATUS-REPORT, hex only). Header-aware part sizes (`capacity`, `build_parts`, `arrange`, `build_status_report` in `sms-pdu.py`). Defaults unchanged (checked PDU-for-PDU against the previous script) | Partial | Still to add: `--dcs HEX` (arbitrary DCS, MWI groups), `--strict-ucs2`, national tables beyond Hindi/Bengali | `android/scripts/tests/test_sms_pdu.py` (pytest, run in CI): round trips through a local decoder, python-gsmmodem-new (ports, 16-bit refs, status reports) and smspdudecoder (concatenation); device-test-plan 27-31 |
| Alphanumeric TP-OA: at most 11 characters, GSM 7-bit packed, TON/NPI 0xD0, length in semi-octets | `sms-pdu.py` `encode_originator` (`(septets*7+3)//4` useful semi-octets) | Compliant | None | `VD-HDFCBK-T` (11 characters) decodes to the same string |
...
P2 text-safety update: every row below was re-checked after the fixes. Paths are relative to `android/`; `unicode/`
means `classify/src/main/kotlin/app/dak/classify/unicode/`. Unicode data is version 17.0.0, generated by
`classify/tools/gen_unicode_tables.py` into `classify/src/main/resources/app/dak/classify/unicode/` (each file's header
records its source files and SHA-256).

| Requirement | Dak implementation (file:line) | Status | Remediation | Test |
|---|---|---|---|---|
| UAX #9: isolate untrusted text (sender names, snippets, subjects, links) so it cannot reorder surrounding UI text; isolates (FSI/PDI) preferred | `unicode/UntrustedText.kt:62-91` wraps in U+2068 FSI … U+2069 PDI (`isolateLtr`: LRI … PDI for URLs and hosts). `app/.../ui/common/text/BidiText.kt:30-45` delegates, so its 26 call sites (inbox, conversation, search, folding, passbook) now get real isolates instead of `BidiFormatter.unicodeWrap`'s embeddings and marks. Composed notification text isolates the sender too: the OTP title and line (`notifications/MessageNotifier.kt:187`), the scam title (`:367`) and summary lines (`NotificationSummaries.kt:81`) | Compliant | Check rendering on a device: Arabic/Urdu names in the inbox and notification shade, RTL UI locale | `UntrustedTextTest`, `app` `BidiTextTest` (Arabic name followed by "₹500") |
| Strip bidi overrides and invisible characters from display names | `unicode/UntrustedText.kt:68-89`: bidi controls, marks and invisible characters removed; ZWJ/ZWNJ kept inside words, now also after a virama or vowel sign (Devanagari half forms such as `क्` + ZWJ + `ष` were broken before) and ZWJ inside emoji sequences | Compliant | None | `UntrustedTextTest`: U+202E + `"gpj.exe"` → `"gpj.exe"`, half forms, Persian ZWNJ, family emoji |
| Bidi controls inside message bodies and extracted links or amounts (for example an RLO that visually reverses a URL or amount) | A stray PDI or RLO can no longer escape an isolate: `isolate` always removes scoped controls first (`UntrustedText.kt:41-65`). Conversation bubbles replace them with U+2060 (same length, so OTP, search and link offsets hold) (`app/.../ui/conversation/MessageText.kt:39`); notifications strip them (`NotificationText.kt`); links end at them (`LinkExtractor.kt`); the link warning shows the raw link as LRI … PDI (`LinkSafety.kt:149`) | Compliant | Entity spans are not individually isolated: an LTR URL inside RTL text follows normal UAX #9 ordering, which is expected | `UntrustedTextTest` (`Pay at ` + U+202E + `https://moc.knabcfdh`), `SecurityTest` (links end at RLO) |
...
| UTS #39 confusables for link hosts (skeleton, brand look-alikes) | `unicode/Confusables.kt:23-42`: UTS #39 skeleton from a table generated from `confusables.txt` 17.0.0 (MA) for Latin, Cyrillic, Greek, Armenian, Cherokee, Devanagari, Bengali and Common/Inherited (2,630 entries; the hand-picked map is gone). `LookalikeDomainChecker.kt:54-58` compares the skeleton of the host the browser resolves with the official domains' skeletons (`looseSkeleton` also drops diacritics to name the brand) | Compliant | Every IDN host is still `LOOKALIKE` (deliberate: SMS phishing uses IDNs almost only as homographs) | `ConfusablesTest`, `TextSafetyTest`: `hdfcbаnk.com`, `іcicibank.com`, fullwidth and mathematical-bold `hdfcbank`, `hdfcbạnk.com`, `xn--` forms |
| UTS #39 mixed-script / restriction-level checks for sender names (MMS `From` encoded strings, e-mail-gateway originators, non-GSM alphanumeric names) | `unicode/ScriptCheck.kt:56-112` (restriction levels, mixed numbers, per-word mixing), `classify/.../SenderNameCheck.kt:32` (mixed script, passes-for-ASCII, case-folded skeleton). `FakeCreditDetector.kt:116` adds `LOOKALIKE_SENDER` when the skeleton contains a bank header token (`НDFCBK`, `ＨＤＦＣＢＫ`, all-Cyrillic `АХІЅВК`) and the new `MIXED_SCRIPT_SENDER` otherwise; `ClassifierPipeline.kt:83-121` treats such senders' links like an unknown number's. DLT headers must be ASCII (`SenderId.kt:82`), so `VM-НDFCBK-S` is not a registered header and `ı` cannot upper-case into one | Compliant | Contact names (the user's own data) are not checked | `ScriptCheckTest`, `TextSafetyTest`: `"НDFC Bank"` (Cyrillic Н), `Ramesh रमेश` not flagged |
| UTS #46 / IDNA2008 processing for the punycode shown to users | `unicode/Uts46.kt:48-81` (nontransitional mapping from `IdnaMappingTable.txt` 17.0.0, NFC, validity criteria, CONTEXTJ, RFC 5893 Bidi rule; CONTEXTO not checked, as UTS #46 allows) and `unicode/Punycode.kt` (RFC 3492). `LinkExtractor.kt:108` uses the WHATWG URL flags and forbidden-host check, so `asciiHost` is what a browser resolves (null when it would refuse the URL, which is then flagged) | Compliant | None | `Uts46Test`: `faß.de` → `xn--fa-hia.de`, IdnaTestV2 17.0.0 (3,650 rows compatible with the JVM's Unicode data); `PunycodeTest` (RFC 3492 vectors) |
| Show the real (ASCII/punycode) host for IDN and userinfo links before opening | `LinkSafety.kt:96-104,152-156` with `unicode/HostDisplay.kt:26`: each label in Unicode only when it passes UTS #46 strictly, is single-script (highly restrictive), does not mix digits, is not a whole-script look-alike of Latin and is in a script the reader reads (device languages, or the IDN TLD's own script: `उदाहरण.भारत`); punycode otherwise (`аpple.com` → `xn--pple-43d.com`) | Compliant | None | `HostDisplayTest`, `app` `LinkSafetyNormalizationTest` |
...
| MASVS-NETWORK | `app/src/main/res/xml/network_security_config.xml` (referenced from the app manifest): cleartext refused for every host, system CAs only (no user CAs, debug included), commented pin-set template for premium servers. Dak does no HTTP itself: MMS goes through the platform `MmsService`, which talks to (often `http://`) MMSCs in the phone process, so no cleartext exception is needed (`docs/release.md`). `MmsSafety.isDownloadableContentLocation` (`MmsSafety.kt:41-55`) | Compliant | Fill in the pin-set (primary and backup SPKI pins, expiration) when premium servers exist | Proxy with a user CA: premium calls fail. `scripts/check-offline-baseline.sh` |
...
| MASVS-RESILIENCE | R8 shrinking, optimisation and obfuscation plus resource shrinking in release builds (`app/build.gradle.kts`, release block); debug stays unminified. Keep rules in `app/proguard-rules.pro`, audited in `docs/release.md` (library consumer rules cover Hilt, Room, WorkManager, kotlinx.serialization and SQLCipher; Dak keeps worker names, persisted `ActionSpec` kind names and enum `valueOf`) | Compliant | Consider Play Integrity for premium APIs | CI builds both release flavours and runs `scripts/check-release-mapping.sh` (R8 ran, persisted names kept); mapping files uploaded as an artifact |
...
15. **Done: UAX #9.** Real FSI/PDI isolates (`classify/.../unicode/UntrustedText.kt`, `BidiText.kt`), scoped
    bidi controls removed before wrapping and neutralised in bubbles (`MessageText.kt`), links shown LRI…PDI (§14).
16. **Done: UTS #46 and UTS #39.** Nontransitional UTS #46 with RFC 3492 Punycode (`unicode/Uts46.kt`,
    `Punycode.kt`); confusables generated from `confusables.txt` 17.0.0 (`unicode/Confusables.kt`,
    `tools/gen_unicode_tables.py`); restriction-level and mixed-script checks for sender names feeding the scam
    signals (`SenderNameCheck.kt`); reader-aware IDN display (`unicode/HostDisplay.kt`). Still to do: a device check
    of isolate rendering in Compose and SystemUI.
...
23. **`scripts/sms-pdu.py` fixture flags:** done: `--ref16`, `--order`, `--shuffle`, `--drop`, `--port`, `--hex`,
    `--nls hi|bn`, `--status-report` (tests in `android/scripts/tests`, run in CI). Still to add: `--dcs`,
    `--strict-ucs2`, more national tables.
...
26. **MASVS hardening:**
    - Done: R8 and resource shrinking in release, with a CI smoke check and mapping upload (`docs/release.md`).
    - Done: network security config (no cleartext, system CAs only); pins to add when premium servers exist.
    - `dataExtractionRules`.
27. **Done: rate-limiter comment.** The `SendRateLimiter` KDoc now says the platform counter lives in the phone
    process and survives Dak's process death, which is why reservations are persisted. Still worth a device check of
    `sms_outgoing_check_*` values per OEM before loosening the window.
28. **Done: scheduled sends while not the default SMS app** stay PENDING and are retried every 15 minutes instead of
    being marked SENT when the sender only held a copy (`automation/ScheduledSendExecutor.kt`). Follow-up: run due
    sends as soon as the role is regained (hook `SmsRoleMonitor`'s regain into `ScheduledSendExecutor.runDue`).
