...
- **P1 URI bug:** fixed for RESPOND_VIA_MESSAGE (`RespondViaMessage`, decode once, keep `&`); the `sms:` intent
  route in `navigation/IntentRoutes.kt` is tracked separately.
...
| `RESPOND_VIA_MESSAGE` URI and body parsing | `sms/HeadlessSmsSendService.kt:30-32` passes the **encoded** scheme-specific part to `sms/RespondViaMessage.kt` (now public): hfields split on `&` before decoding, each value percent-decoded once (UTF-8, `+` kept, malformed escapes kept literally), `body` matched case-insensitively, recipients split on `,` or `;` and decoded. `EXTRA_TEXT` still wins | Compliant | `navigation/IntentRoutes.kt` can reuse `RespondViaMessage.recipients/body` with `uri.encodedSchemeSpecificPart` (row above) | `RespondViaMessageUriTest` (`sms:+1?body=a%26b%25c` → `a&b%c`) |
...
8. **`sms:` / `smsto:` body parsing:** done for RESPOND_VIA_MESSAGE (`sms/RespondViaMessage.kt`, public, takes the
   encoded SSP). `navigation/IntentRoutes.kt:126-138` should switch to it.
